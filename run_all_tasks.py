...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            ...
            "results/results_easy_task/, results/results_medium_task/, results/results_hard_task/."
        )
    )
    ap.add_argument(
        "--mixed",
    ...
    ap.add_argument(
        ...
        help="Comma-separated subset of tasks to run: easy,medium,hard (default: easy,medium,hard)",
    )
    args = ap.parse_args()
    ...
    # Write results in a results folder at the repo root
    results_easy = repo_root / "results" / "results_easy_task"
    results_medium = repo_root / "results" / "results_medium_task"
    results_hard = repo_root / "results" / "results_hard_task"

    tasks = {t.strip().lower() for t in str(args.tasks).split(",") if t.strip()}
    unknown = sorted(t for t in tasks if t not in {"easy", "medium", "hard"})
    if unknown:
        raise SystemExit(
            f"Unknown --tasks values: {unknown}. Expected subset of easy,medium,hard"
        )
    ...
    if "medium" in tasks:
        ...
        for method in ["kmeans", "agglomerative", "dbscan"]:
            ...
            # DBSCAN is sensitive to density; lowering min_samples reduces the
            # chance that everything becomes noise (-1).
            if method == "dbscan":
        ...
        # Re-aggregate after baselines are generated.
        aggregate_metrics(results_medium, results_medium / "medium_task_comparison.csv")
